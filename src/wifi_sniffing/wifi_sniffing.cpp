...
void WiFiSnifferPacketHandler(void* buff, wifi_promiscuous_pkt_type_t type) {
    ...
    if ((hdr->frame_ctrl & 0xFC) == 0x40) {
        ...
        if (isOUIValid(binOUI)) {
            ...
            if(uniqueDevicesBuffer[bufferIndex].find(addrStr) == uniqueDevicesBuffer[bufferIndex].end()) {
                for (int i = 0; i <= rssiIndex; i++) {
                    N_v[bufferIndex][i]++;
                }

                uniqueDevicesBuffer[bufferIndex].insert(addrStr);
            }

        } else {
            // Update randomized RSSI counters
            if(randomDevicesBuffer[bufferIndex].find(addrStr) == randomDevicesBuffer[bufferIndex].end()) {
                for (int i = 0; i <= rssiIndex; i++) {
                    N_r[bufferIndex][i]++;
                }

                randomDevicesBuffer[bufferIndex].insert(addrStr);
            }
        }
    }
}
